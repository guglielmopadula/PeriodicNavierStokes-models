...
all_points_super=torch.concatenate((time_super.unsqueeze(0).unsqueeze(-1).repeat(4*4096,1,1),points_super.unsqueeze(-2).repeat(1,100,1)),dim=2)
...
class DeepONet(nn.Module):
    def __init__(self,
        ...
        self.branch_net=nn.Sequential(nn.Linear(num_start_points,100),nn.BatchNorm1d(100),nn.ReLU(),nn.Linear(100,100), nn.BatchNorm1d(100),nn.ReLU(),nn.Linear(100,100),nn.BatchNorm1d(100),nn.ReLU(), nn.Linear(100,medium_size))
        self.trunk_net=nn.Sequential(nn.Linear(3,100),nn.BatchNorm1d(100), nn.ReLU(), nn.Linear(100,100), nn.BatchNorm1d(100),nn.ReLU(),nn.Linear(100,100), nn.BatchNorm1d(100),nn.ReLU(), nn.Linear(100,medium_size))
...
for epoch in trange(Epochs):
    ...
    for batch in train_dataloader:
        ...
        v=v.reshape(v.shape[0],-1)
        u=u.reshape(u.shape[0],-1)
        pred=model.forward_eval(v[:,index_subset],all_points)
...
with torch.no_grad():
    ...
    v=v.reshape(v.shape[0],-1)
    u=u.reshape(u.shape[0],-1)
    pred=model.forward_eval(v[:,index_subset],all_points_super)
    u=u.reshape(u.shape[0],-1)
    pred=pred.reshape(pred.shape[0],-1)
    u=u.cpu().numpy()
    pred=pred.cpu().numpy()
