...
class Operator():
    def __init__(self,m): #m is the number of sensors
        self.model_trunk=ANN(2,100).cuda()
        self.model_branch_1=ANN(m,100).cuda()
        self.model_branch_2=ANN(1,100).cuda()
        self.m=m
    
    def fit(self,x,points,times,A,B,C):
        self.indices=np.random.permutation(64*64)[:self.m]
        self.model_branch_1.fit(x[:,self.indices],A,1000)
        self.model_branch_2.fit(times,C,1000) #time is last
        self.model_trunk.fit(points,B,1000)
